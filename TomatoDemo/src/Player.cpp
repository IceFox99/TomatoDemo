...
void Player::FixedUpdate(size_t row, size_t col, Level& level) {
	...
	if (Tomato::Input::IsKeyPressed(TM_KEY_LEFT) || Tomato::Input::IsKeyPressed(TM_KEY_A)) {
		newState = State::MovingLeft;
		m_LastWasLeft = true;
	} else if (Tomato::Input::IsKeyPressed(TM_KEY_RIGHT) || Tomato::Input::IsKeyPressed(TM_KEY_D)) {
		newState = State::MovingRight;
		m_LastWasLeft = false;
	}

	if (Tomato::Input::IsKeyPressed(TM_KEY_UP) || Tomato::Input::IsKeyPressed(TM_KEY_W)) {
		secondaryState = newState;
		newState = State::MovingUp;
	} else if (Tomato::Input::IsKeyPressed(TM_KEY_DOWN) || Tomato::Input::IsKeyPressed(TM_KEY_S)) {
		secondaryState = newState;
	...
	bool ctrlPressed = Tomato::Input::IsKeyPressed(TM_KEY_LEFT_CONTROL) || Tomato::Input::IsKeyPressed(TM_KEY_RIGHT_CONTROL);
	size_t oldRow = row;
	size_t oldCol = col;
	switch (m_State) {
...
void Player::Animate(Tomato::Timestep ts) {
